...
curl -sSfL "https://github.com/katbyte/prawn/archive/refs/tags/${TAG}.tar.gz" -o "$tmp/src.tar.gz"
SHA256="$(sha256sum "$tmp/src.tar.gz" | cut -d' ' -f1)"

cat > "$tmp/prawn.rb" <<FORMULA
# typed: false
# frozen_string_literal: true

# This file is generated by the prawn release workflow - do not edit by hand.
class Prawn < Formula
  ...
  url "https://github.com/katbyte/prawn/archive/refs/tags/${TAG}.tar.gz"
  sha256 "${SHA256}"
  ...
  test do
    assert_match "${TAG}", shell_output("#{bin}/prawn version")
  end
end
FORMULA
...
cp "$tmp/prawn.rb" "$tmp/tap/Formula/prawn.rb"
cd "$tmp/tap"
git config user.name "prawn-release"
git config user.email "prawn-release@katbyte.me"
git add Formula/prawn.rb
git diff --cached --quiet && { echo "==> formula unchanged"; exit 0; }
git commit -m "prawn ${TAG}"
git push
echo "==> done"
